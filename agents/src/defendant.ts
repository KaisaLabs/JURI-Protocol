...
function getConfig(): AgentConfig {
  ...
  return {
    ...
    address: getRoleAddress("defendant"),
    privateKey: getRolePrivateKey("defendant"),
    ...
    controlPort: getControlPort("defendant"),
  };
}
...
class DefendantAgent extends BaseAgent {
  ...
  async start(): Promise<void> {
    ...
    await this.reportStatus("ready", `Defendant seeded for case #${this.caseId}`);
    this.log(`📋 Case #${this.caseId}: "${this.disputeQuestion}"`);

    // === Round 1: Counter to opening ===
    await this.reportStatus("debating", "Defendant waiting for plaintiff opening argument");
    ...
    await this.reportEvidence("argument", 1, ref1, "Defendant first counter stored");
    await this.sendTo("plaintiff", "COUNTER_ARGUMENT", counter1, [ref1.ref]);
    ...
    await this.reportEvidence("argument", 2, ref2, "Defendant second counter stored");
    await this.sendTo("plaintiff", "COUNTER_ARGUMENT", counter2, [ref2.ref]);
    ...
    await this.reportEvidence("argument", 3, ref3, "Defendant final counter stored");
...
Summarize why the plaintiff's case fails and why you should win.`;
    ...
    await this.reportStatus("awaiting_verdict", "Defendant sending closing statement to judge");
    await this.reportEvidence("closing", 3, ref3, "Defendant closing statement references submitted evidence");
    ...
    await this.reportStatus("resolved", `Defendant received verdict ${verdict.result}`, {
