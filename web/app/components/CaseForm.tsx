  ...
  return (
    <div className="space-y-4">
      <textarea value={dispute} onChange={e => setDispute(e.target.value)}
        ...
        rows={3} className="w-full bg-[#06060b] border border-[#1e2030] rounded-lg px-4 py-3 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-[#00d4ff] transition-colors resize-none" />
      <div className="flex gap-4 items-end">
        <div className="w-32">
          ...
          <input type="number" value={stake} onChange={e => setStake(e.target.value)} step="0.01" min="0.001"
            className="w-full bg-[#06060b] border border-[#1e2030] rounded-lg px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-[#00d4ff] transition-colors" />
        </div>
        <button onClick={() => onSubmit({ id: 0, dispute: dispute.trim(), stake })} disabled={loading || !dispute.trim()}
          className="px-8 py-2.5 bg-[#00d4ff] text-black font-bold rounded-lg hover:bg-[#d4b55a] disabled:opacity-40 transition-all text-sm">
