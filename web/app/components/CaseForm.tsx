...
import type { CaseState, CaseStatus } from "../page";
...
export default function CaseForm({ onCaseCreated, currentCase }: CaseFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Store dispute to 0G KV
    // 2. Deploy/dispatch to smart contract
    // 3. Notify agents via AXL

    // For now, create the case state
  ...
  const isActive = currentCase && currentCase.status !== "RESOLVED";
      ...
      {isActive ? (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-600 text-center">
            Agents are debating. Watch the live feed below.
          </p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder='e.g. "Will ETH be above $3,000 on May 5, 2026?" or "Is DeFi safer than CeFi?"'
              rows={3}
